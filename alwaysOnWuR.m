...
while true
    ...
    % Wake-up radio is checking for a signal every second
    disp(['Time ' num2str(currentTime) 's: Wake-up radio is checking for a signal.']);
    fprintf(logFile, 'Time %ds: Wake-up radio is checking for a signal.\n', currentTime);
    
    % Randomly trigger reconnection events to mimic connection loss
    if ~isReconnectionRequired && rand() > 0.9999 % Randomly initiate reconnection
    ...
    if detectionChance > threshold || isReconnectionRequired
        ...
        % WuR is now active, processing the wake-up signal
        if isReconnectionRequired
            ...
            disp(['Time ' num2str(currentTime) 's: Wake-up signal detected. WuR is active and processing the wake-up signal.']);
            fprintf(logFile, 'Time %ds: Wake-up signal detected. WuR is active and processing the wake-up signal.\n', currentTime);
        end

        % Pause for 1 second to simulate the WuR being active while processing the wake-up signal
        pause(1); % Simulate the 1 second delay

        % After the 1-second delay, the BLE device wakes up and begins communication
        currentTime = currentTime + 1; % Increment current time by 1 second after the pause
        ...
        if ~isCacheInitialized || isReconnectionRequired
            fprintf("Generated Bluetooth LE LL advertising channel PDU.\n");
            ...
            clientAdvPDU.UsedChannels = [4 5 14 16 21 28 36]; 
            ...
            % GATT Client: Central Device receives the service discovery response
            %fprintf(logFile, 'Time %ds: Central Device (GATT Client) is receiving the service discovery response.\n', currentTime);
            ...
            chrsticDiscReqCfg
            [chrsticDiscRespPDU, chrsticDiscReqCfg, gattServer] = receiveData(gattServer, receivedPDU);
            cache.characteristics = chrsticDiscReqCfg; % Store in cache
            ...
            [chrsticDescDiscRespPDU, chrsticDescDiscReqCfg, gattServer] = receiveData(gattServer, receivedPDU);
            cache.descriptors = chrsticDescDiscReqCfg; % Store in cache
            ...
            % GATT Client: Central Device receives heart rate notification
            %fprintf(logFile, 'Time %ds: Central Device (GATT Client) is receiving notification enablement response.\n', currentTime);
        ...
        % GATT Client: Central Device receives heart rate measurement notification
        ...
        if all(validPackets)
            helperBLEExportToPCAP(pcapPackets, 'HeartRateImplant(1).pcap');
            fprintf("Open generated pcap file 'HeartRateImplant(1).pcap' in a protocol analyzer to view the generated frames.\n");
        else
            warning('Some PDUs are empty, skipping those in PCAP export.');
            helperBLEExportToPCAP(pcapPackets(validPackets), 'HeartRateImplant(1).pcap');
            fprintf("Open generated pcap file 'HeartRateImplant.pcap(1)' in a protocol analyzer to view the generated frames.\n");
        end
    ...
    % Pause for simulation purposes (simulate real-time behavior)
